/* ranoise32c (best in PractRand) test program
 ...
 */
static inline int32_t ranoise32c(uint32_t n) {
	uint32_t s = n * FIBH32;
	s ^= s >> 15;
	s = MUVAROR32(s, s >> 27, 16);
	s ^= s >> 14;
	return s;
}
 ...
 */
static inline int32_t ranoise32c_next(uint32_t *restrict pos) {
